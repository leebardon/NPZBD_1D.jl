...
function rk4(ntemp, ptemp, ztemp, btemp, dtemp, otemp, prms, t)
    ...
    track_n1 = n .+ dt/2 .* dNdt1
    track_p1 = p .+ dt/2 .* dPdt1
    track_z1 = z .+ dt/2 .* dZdt1
    track_b1 = b .+ dt/2 .* dBdt1
    track_d1 = d .+ dt/2 .* dDdt1
    track_o1 = o .+ dt/2 .* dOdt1
    ...
    track_n2 = n .+ dt/2 .* dNdt2
    track_p2 = p .+ dt/2 .* dPdt2
    track_z2 = z .+ dt/2 .* dZdt2
    track_b2 = b .+ dt/2 .* dBdt2
    track_d2 = d .+ dt/2 .* dDdt2
    track_o2 = o .+ dt/2 .* dOdt2
    ...
    n .+= (dNdt1 .+ 2 .* dNdt2 .+ 2 .* dNdt3 .+ dNdt4) .* (dt / 6)
    p .+= (dPdt1 .+ 2 .* dPdt2 .+ 2 .* dPdt3 .+ dPdt4) .* (dt / 6)
    z .+= (dZdt1 .+ 2 .* dZdt2 .+ 2 .* dZdt3 .+ dZdt4) .* (dt / 6)
    b .+= (dBdt1 .+ 2 .* dBdt2 .+ 2 .* dBdt3 .+ dBdt4) .* (dt / 6)
    d .+= (dDdt1 .+ 2 .* dDdt2 .+ 2 .* dDdt3 .+ dDdt4) .* (dt / 6)
    o .+= (dOdt1 .+ 2 .* dOdt2 .+ 2 .* dOdt3 .+ dOdt4) .* (dt / 6)
